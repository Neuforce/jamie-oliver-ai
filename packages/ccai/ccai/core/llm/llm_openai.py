...
class OpenAILLM(BaseLLM):
    """
    An implementation of BaseLLM that interacts with OpenAI's language db,
    ...
    @observe(name="llm_generation_openai")
    async def invoke(
    ...
    def _transform_messages(self, messages: List[Message]) -> List[Dict[str, Any]]:
        """
        Transform messages into the format expected by the OpenAI API.
    ...
    def _generate_function_schemas(
        ...
        """
        Generate function schemas for the OpenAI API from the registered functions.
    ...
    def _convert_schema(func: RegisteredFunction) -> Dict[str, Any]:
        """
        Convert a RegisteredFunction into a function schema for the OpenAI API.
